module hurricane_wind_mod
  ...
  subroutine hurricane_wind_init(Hurricane,Atm,time_ext)    
    ...
    integer :: unit,ierr,io,I_storm
    integer :: trackunit,readunit,outunit,logunit,month,day,hour,minute,second,date,nlon,nlat,eof,eofv
    real julday_ext
...
17  format(A19,I6,1x,I4,1x,I3,A1,1x,I4,A1,1x,I3,1x,I3,3I5,  &
    ...
    do I=1,PLN
      do J=1,PLN
        ...
        rmw_vit(I,J)=0
        do I_storm=1,5
          garb_vit(I,I_storm,J)=0.
        end do
        do I_storm=1,4
          Rd1_vit(I,I_storm,J)=0
          Rd2_vit(I,I_storm,J)=0
        end do
      end do
    end do
    ...
    do while(eof.eq.0)
      ...
      if (stormname.ne.'000') then
        open(trackunit,file='INPUT/track_'//stormname,status='old')
        eofv=0.
        I=1
        do while(eofv.eq.0)
          read(trackunit,17) name_vit(I_storm,I),date_vit(I_storm,I),hour_vit(I_storm,I),lat_vit(I_storm,I),lns_vit(I_storm,I),long_vit(I_storm,I),lew_vit(I_storm,I),garb_vit(I_storm,:,I),mx_vit(I_storm,I),rmw_vit(I_storm,I),Rd1_vit(I_storm,:,I),Rd2_vit(I_storm,:,I)
          if(date_vit(I_storm,I).ne.0) then
            I=I+1
          else
            eofv=1.
          end if
...
!
    ...
    integer :: is,ie,js,je,isize,jsize,remap_method,Iread,I_storm
    integer*4 sdate
    integer slat,slong,smx,srmw,shour
    integer sgarb(5),sRd1(4),sRd2(4)
    ...
    real juldays,juldaye,end
        
    integer outunit,month,day,hour,minute,second,date
    integer :: readunit, eof, logunit, readunits, eofs,io
    character*3 :: stormname
    character*19 sname
    character*1 slns,slew
  ...
  !-----------WRAP THE PROFILE CALL IN A LOOP WHICH READS FROM storm_list, and passes as a pathname----------------
    I_storm=1
    do while(date_vit(I_storm,1).ne.0)
      call hurricane_wind_profile(hu,hv,hlon,hlat,I_storm)
      I_storm=I_storm+1
...
!
  ...
  subroutine hurricane_wind_profile(u,v,hlon,hlat,I_storm)
    use hurricane_wind_common
    
    real, intent(inout)      :: u(im,jm),v(im,jm),hlon(im),hlat(jm)
    integer, intent(in)  :: I_storm
...
!----------------------- Reading message file --------------------
    I=1
    do while(date_vit(I_storm,I).ne.0)
      date=date_vit(I_storm,I)*100+hour_vit(I_storm,I)/100
      call hurricane_wind_date2day(year,julday,date)
      TM(i)=julday
      X(i)=long_vit(I_storm,I)/10.
      if(lew_vit(I_storm,I).eq.'W') X(i)=-X(i)
      if(lew_vit(I_storm,I).eq.'E'.and.X(i).gt.80.) X(i)=X(i)-360.
      Y(i)=lat_vit(I_storm,I)/10.
      if(lns_vit(I_storm,I).eq.'S') Y(i)=-Y(i)
      if(lns_vit(I_storm,I).eq.'S') ns_flag=lns_vit(I_storm,I)
      PRES(i)   =real(garb_vit(I_storm,3,I))
      PRES0(i)  =real(garb_vit(I_storm,4,I))
      RADCLS(i) =real(garb_vit(I_storm,4,I))
      WSPMAX(i) =real(mx_vit(I_storm,I))
      RMAXa(i)  =real(rmw_vit(I_storm,I))
      do n=1,4
        n1=n+(1-mod(n,2))*sign(2,3-n)
        R18v(i,n)=Rd1_vit(I_storm,n1,I)
        R26v(i,n)=Rd2_vit(I_storm,n1,I)
